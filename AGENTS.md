...
- **Host toggle `guestInvitesEnabled`** (composer `GuestInvitesField`, "Teilnehmer" section,
  default **OFF** — widening the audience exposes the host's plans and pin, so it stays a
  deliberate host decision). Unlike the Sichtbarkeit picker it IS editable in edit mode:
  toggling changes only the future invite mechanism, never the current audience; turning it
  off never removes already-invited guests.
...
- **The circle/ring and woven-ribbon concepts are retired.** Their legacy implementation was removed; the native splash stays a plain `#070910` stage until React renders the current wordmark.
