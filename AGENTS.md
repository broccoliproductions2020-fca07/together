...
- `PostfachSheet` owns joined rooms, notifications, friendship requests, Journey and Safety notices. Its joined activity list derives from `ChatProvider.joinedIds`, resolves activity ids through `findActivityById` or groups through `getGroup`, and sorts by `lastMessage.at ?? createdAt`.
- Opening a room passes the resolved activity to `MapScreen`, which opens the existing chat Modal.
...
- **Location default:** Always `CURRENT_LOCATION_PLACE` (Aktueller Standort, `utils/currentPlace.ts`) — never "Ort noch offen"
