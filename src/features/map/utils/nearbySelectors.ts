import { mockNearbyFriends } from '@/data/mock';
...
export function selectNearbyFriends(radiusKm: number): NearbyFriend[] {
...
export function selectFriendsWithoutLocation(): NearbyFriend[] {
  return mockNearbyFriends.filter((f) => f.locationVisibility === 'none');
}

/** The single number shown on the pill AND as the sheet's "in deiner Nähe" count. */
export function selectNearbyCount(radiusKm: number): number {
  return selectNearbyFriends(radiusKm).length;
}
