...
function connectGoogleAccount() {
  const started = dialog.showMessageBoxSync(dashboardWindow, {
    ...
    detail:
      'Nach erfolgreicher Anmeldung speichert Together nur die verschlüsselte OAuth-Anmeldung für diesen Windows-Account. Kein Google-Passwort wird gespeichert.',
  ...
  login.once('close', (code) => {
    ...
    try {
      ...
      dialog.showMessageBoxSync(dashboardWindow, {
        ...
        message:
          'Die Anmeldung wurde für diesen Windows-Account verschlüsselt gespeichert. Together startet jetzt neu.',
      });
      restartDesktop();
    } catch (error) {
      rmSync(gcloudConfigPath, { recursive: true, force: true });
...
function disconnectGoogleAccount() {
  if (!existsSync(vaultPath())) {
    dialog.showMessageBoxSync(dashboardWindow, {
      ...
      message: 'Für Together Betrieb ist aktuell keine Google-Anmeldung gespeichert.',
    });
    return;
  }
...
app.whenReady().then(async () => {
  try {
    ...
    showError('Together Betrieb konnte nicht starten', error);
    app.quit();
  }
});
